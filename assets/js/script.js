...
let restart_div = $("#restart-div");
let restart_btn = $("#restart");

// load sounds
var sound = new Array();
    sound[0] = new Audio('http://jonkantner.com/experiments/stick_fight/sounds/hit.ogg');
    sound[1] = new Audio('http://jonkantner.com/experiments/stick_fight/sounds/miss.ogg');
...
for (var i = 0; i <= maxFrames; ++i) {
    ...
    player1Sprites[i].src = "http://jonkantner.com/experiments/stick_fight/sprites/player1/player" + i + ".svg";
    if (i == maxFrames) {
        anim1 = function(){
...
for (var j = 0; j <= maxFrames; ++j) {
    ...
    player2Sprites[j].src = "http://jonkantner.com/experiments/stick_fight/sprites/player2/player" + j + ".svg";
    if (j == maxFrames) {
        anim2 = function(){
...
function incKO(winner, winnerCounter) {
	if(winner.kills <= 1){
		...
		console.log(winner.kills);
	} else {
		console.log(winner.name);
		restart_div.slideDown();
		document.getElementById('show-winner').innerHTML = `${winner.name} is the Winner!`;
		restart_btn.focus();
	}
}
...
function respawn(newLife, healthToFill) {
  ...
  healthToFill.style.background = "#67D1E3";
}
